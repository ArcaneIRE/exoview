...
const MOVE_SPEED_FAST = 50
const MOVE_SPEED_SLOW = 5
...
func _input(event):
	...
	if event.is_action_pressed("escape"):
		if mouseCaptured: #If escape is pressed while in fp mode, return mouse
			Input.set_mouse_mode(Input.MOUSE_MODE_VISIBLE)
			mouseCaptured = false
		else: #If escape is pressed while not in fp mode, exit application
			get_tree().quit()		
	elif event.is_action_pressed("left_mouse_click"):
		if !mouseCaptured: #if left mouse pressed while not in fp, enter fp mode
...
func handleMouseMovement(delta:float):
...
func handleMoveControl(delta:float):
